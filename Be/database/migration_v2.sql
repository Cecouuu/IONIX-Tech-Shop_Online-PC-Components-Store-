...
ALTER TABLE users
  ADD COLUMN IF NOT EXISTS is_owner TINYINT(1) NOT NULL DEFAULT 0 AFTER is_admin;
...
ALTER TABLE products ADD COLUMN IF NOT EXISTS category VARCHAR(80) NOT NULL DEFAULT 'Components' AFTER name;
ALTER TABLE products ADD COLUMN IF NOT EXISTS image_url VARCHAR(600) NOT NULL DEFAULT '' AFTER description;
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  guest_address VARCHAR(255) NOT NULL DEFAULT '',
...
ALTER TABLE orders ADD COLUMN IF NOT EXISTS guest_name VARCHAR(120) NOT NULL DEFAULT '' AFTER user_id;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS guest_email VARCHAR(190) NOT NULL DEFAULT '' AFTER guest_name;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS guest_phone VARCHAR(40) NOT NULL DEFAULT '' AFTER guest_email;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS guest_address VARCHAR(255) NOT NULL DEFAULT '' AFTER guest_phone;
...
SELECT 'Admin', 'admin@techstore.local', '$2y$10$voNtYGuAcHxC3cfv/Uai3e.XquBTA82JsA.CFkuaVXixOupszjaHK', 1, 1
WHERE NOT EXISTS (
  SELECT 1 FROM users WHERE email = 'admin@techstore.local'
);
UPDATE users SET is_owner = 1, is_admin = 1 WHERE email = 'admin@techstore.local';
